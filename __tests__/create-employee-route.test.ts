...
describe("create-employee route", () => {
  ...
  it("returns 400 for invalid email", async () => {
    const res = await POST(makeRequest({ email: "bad", password: "secret1", name: "John" }))
    expect(res.status).toBe(400)
    const json = await res.json()
  ...
  it("returns 401 when not logged in", async () => {
    ...
    const res = await POST(makeRequest({ email: "john@example.com", password: "secret1", name: "John" }))
  ...
  it("creates user successfully and returns ok", async () => {
    ...
    const res = await POST(makeRequest({ email: "john@example.com", password: "secret1", name: "John", type: "fulltime" }))
  ...
  it("falls back to insert into public.users when trigger did not run", async () => {
    ...
    const res = await POST(makeRequest({ email: "john@example.com", password: "secret1", name: "John", type: "intern1" }))
    expect(res.status).toBe(200)
    const json = await res.json()
    expect(json.ok).toBe(true)
    expect(json.user_id).toBe("new-user-id")
